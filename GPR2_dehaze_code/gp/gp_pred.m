...
if iscell(gp) || numel(gp.jitterSigma2)>1 || isfield(gp,'latent_method')%不运行
...
tn = size(x,1);
if nargout > 2 && isempty(yt)%不运行
  lpyt=[];
end

if isfield(gp.lik, 'nondiagW') && ~ismember(gp.lik.type, {'LGP' 'LGPC'})%不运行
...
switch gp.type
  case 'FULL'%运行
    ...
    if ~isfield(gp.lik, 'nondiagW') || ismember(gp.lik.type, {'LGP' 'LGPC'})%运行
      %evaluate a = C\y;
      % -------------------
      [tmp, C]=gp_trcov(gp,x);
      ...
      if isfield(gp,'derivobs') && gp.derivobs==1%不运行
