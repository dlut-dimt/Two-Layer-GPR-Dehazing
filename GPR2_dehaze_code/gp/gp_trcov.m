...
if length(gp.cf)==0 || (nargin>2 && ~isempty(predcf) && predcf(1)==0)%不运行
  K=[];
  C=[];
...
if ~(isfield(gp,'derivobs') && gp.derivobs)%运行
  
  switch gp.type
    case {'FULL' 'FIC' 'PIC' 'PIC_BLOCK' 'CS+FIC' 'VAR' 'DTC' 'SOR'}%运行
      [n,m]=size(x1);
      n1 = n+1;
      ncf = length(gp.cf);
