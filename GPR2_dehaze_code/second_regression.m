...
%========================================================
function [data0 pre_var]=second_regression(local_stru)

st=clock;
disp('GP2 with Gaussian noise model...')

data1=importdata('neighbor_feature.mat'); %邻域训练数据
data1=data1/255;
data0=local_stru; %二层测试数据
[hh1 ww]=size(data1);
[hh0 ww0]=size(data0);
data=[data1;data0];
hh_train=hh1;
...
gp=importdata('gp_trans.mat'); %gp模型
...
pre_var=reshape(Varft_map,1,hh_test);
times_var=1/max(pre_var);
pre_var=pre_var*times_var;
data0(:,ww)=pre_y;
